...
def test_create_pod_sends_expected_body_and_returns_response(monkeypatch):
    ...
    assert captured["body"]["image"] == "ghcr.io/example/hangar:latest"
...
def test_create_pod_raises_when_volume_given_with_no_data_center_id():
    with pytest.raises(ValueError):
        create_pod(
            name="hangar-test",
            image="public/image:latest",
...
def test_create_network_volume_sends_expected_body(monkeypatch):
    ...
    assert captured["body"] == {"name": "jlt-gpu-run", "size": 10, "dataCenter": "US-WA-1"}
